...
| `frames_dropped` | int64 · frames · `counter` | **Driver** | Frames the decoder discarded **before output** — undecodable (corruption / unsupported) or shed under decode overload. **Excludes** presentation-side drops (Section 3.2). A frame here is not also in `frames_decoded`. |
...
| `decode_errors` | int64 · events · `counter` | **Driver** | Audio decode-error occurrences (distinct axis from `frames_dropped`, as Section 3.1). Decoder-internal. |
...
| `underflow_duration_ms` | int64 · ms · `counter` | **Driver** | Cumulative time the audio path was starved of decoded data (Σ episode start→clear). |
...
| `last_decode_error_pts_ms` | int64 · ms · `current` | **Driver** | Stream PTS at detection of the most recent audio decode error. As `av.video_decoder.last_decode_error_pts_ms`. |
| `last_decode_error_reason` | int64 · none · `current` | **Driver** | Closed-vocabulary classification of the most recent audio decode error. |
| `last_decode_error_vendor_code` | int64 · none · `current` | **Driver** | The SoC's own code for the same fault, uninterpreted. |
| `last_underflow_trigger` | int64 · none · `current` | **Driver** | Cause of the most recent audio underflow episode, same vocabulary as the video sink. |
| `last_underflow_duration_ms` | int64 · ms · `current` | **Driver** | Length of the most recent completed audio underflow episode. |
...
| `denied_count` | int64 · events · `counter` | **Rialto** | `canCreateSession()` calls that returned unsatisfiable. Lets a QoE consumer distinguish **"playback never started"** from **"playback started and then failed"** — two very different customer experiences that are indistinguishable from playback counters alone. |
