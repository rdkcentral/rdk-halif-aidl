...
| **HAL.RINGBUF.21** | An overflow may overwrite a region the consumer currently holds; the implementation shall not defer, shorten or refuse the producer's `acquire()` to protect it.       | Overflow is *defined* as the producer acquiring over the consumer's read position. |
...
| **HAL.RINGBUF.28** | `requestFlush()` shall throw `EX_ILLEGAL_STATE`, leaving the buffer untouched, when the consumer still holds an unreleased acquire result.        | A held region would point at reset positions.              |
| **HAL.RINGBUF.29** | After a flush the consumer's `acquire()` shall return `null` until data written after the flush has been committed.                              | Enforced by the implementation, not by producer timing.    |
| **HAL.RINGBUF.30** | The ring buffer shall invalidate data released by the producer through any acquire result obtained before or during a flush.                     | Covers the asynchronous window; a late release is not an error. |
| **HAL.RINGBUF.31** | The implementation shall report peer loss through `onError()` with `PRODUCER_DISCONNECTED` or `CONSUMER_DISCONNECTED`.                             |                                                            |
| **HAL.RINGBUF.32** | On peer loss the implementation shall release that peer's registration and any outstanding acquires, returning the slot to its unregistered state. | A crashed peer must not permanently block re-registration. |
| **HAL.RINGBUF.33** | An `onError()` call shall carry a human-readable message; `IMPLEMENTATION_ERROR` shall always do so.                                               |                                                            |

### Ownership and Access

| #                  | Requirement                                                                                                                             | Comments                                     |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| **HAL.RINGBUF.34** | `IRingBuffer` shall not be registered with the service manager and shall not be returned to a producer or a consumer.                   | Implementation-internal by design.           |
| **HAL.RINGBUF.35** | The owning HAL component shall publish only the endpoint a client needs — an `IRingBufferSink` or an `IRingBufferSource`.               | Clients never configure the buffer.          |
| **HAL.RINGBUF.36** | `IRingBuffer.getInfo()` shall return the current size, readable byte count and overflow setting, including before any client registers. | Lets the owner read back what it configured. |

### Peer Death

| #                  | Requirement                                                                                                                     | Comments                                   |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| **HAL.RINGBUF.37** | The implementation shall register a death recipient on each listener passed to `registerProducer()` / `registerConsumer()`.     | The listener is the client-hosted binder.  |
| **HAL.RINGBUF.38** | On producer death the implementation shall discard any acquire result the producer held, since it was never released.           | Uncommitted bytes must not become visible. |
| **HAL.RINGBUF.39** | On consumer death the implementation shall release any acquire results the consumer held, returning that space to the producer. |                                            |
| **HAL.RINGBUF.40** | Cleanup on peer death shall require no call from the surviving client; the slot shall be reusable without an unregister call.   | Recovery must not depend on the dead peer. |
...
**An outstanding acquire is not protected.** `OVERFLOW` is raised precisely when the producer acquires a region that
covers the consumer's read position — that is, the oldest result the consumer has not yet released. Holding a result
therefore reserves nothing: the bytes behind it can be rewritten while the consumer is reading them. The consumer
gets a warning, not exclusive use of the region, and on an overflowing buffer it must be prepared for what it has
already read to be stale or torn.
...
The consumer **must** release every acquire result it holds before calling `requestFlush()`; the call throws
`EX_ILLEGAL_STATE` otherwise and the buffer is left untouched. This is enforced rather than advised because a region
acquired before the flush refers to positions the flush has reset, so reading it afterwards yields arbitrary bytes —
and the consumer has no way to detect that from the result it holds.

After the flush there is nothing to read until data written after the flush has been committed. `acquire()` returns
`null` in the meantime, and data the producer releases through an acquire result it obtained **before or during** the
flush is invalidated by the ring buffer rather than made readable.

**That invalidation is what makes the flush safe, not the producer's timing.** The flush is asynchronous, so the
producer may still be writing when it takes effect and cannot observe the moment it happened — requiring it to stop at
exactly the right instant would be unimplementable. Covering acquire results obtained *during* the flush, not merely
before it, is what closes that window.

The producer's part is best effort. On `onFlushRequested()` it should stop writing into the region it holds, release
it committing nothing — `release(id, 0)` — and acquire a fresh region before writing anything new. A late release is
not an error, because the release is invalidated anyway. Abandoning promptly shortens the gap during which the
