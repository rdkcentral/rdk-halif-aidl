...
The stage that applies the trim converts back to whole samples as `round(ns × sample_rate ÷ 1_000_000_000)`. At 48 kHz, `21_333_333` ns is 1024 samples; truncating instead would leave one priming sample in the frame. Where a leading or trailing trim is split across frames, each frame's value is rounded independently, and the split is chosen so the per-frame sample counts sum to the total the container signalled.
...
samples discarded     = round(21_333_333 × 48000 / 1_000_000_000) per frame
                      = 1024 + 1024 = 2048   (the full priming)
```
