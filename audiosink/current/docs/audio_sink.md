...
!!! info References
    |||
    |-|-|
    |**Interface Definition**|[audiosink/current](https://github.com/rdkcentral/rdk-halif-aidl/tree/main/audiosink/current)|
...
Typically an RDK middleware GStreamer audio sink element will work with a single `IAudioSink` instance and pass it PCM audio in [AV Buffer](../avbuffer/av_buffer.md) handles for mixing.
...
Audio buffers entering the Audio Sink shall be delivered as AV Buffer handles (allocated from either an [AV Buffer](../avbuffer/av_buffer.md) audio pool or the audio frame pool) with a presentation timestamp and metadata describing the audio frame through the `IAudioSinkController.queueAudioFrame()` function.
