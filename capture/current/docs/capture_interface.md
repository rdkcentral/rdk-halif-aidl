...
| **HAL.CAPTURE.8** | Shall carry each frame's presentation time unaltered in `VideoFrameView.presentationTimeNs`.| It is the frame's only timing reference. A captured frame goes to the client's scene rather than to a display plane, so the client presents it against the clock its audio path already runs on. |
...
All of these are in `com.rdk.hal.capture`. The plane itself is described in [Plane Control](../plane_control.md).
