...
`tests/yocto/meta-rdk-halif/` whose recipes are generated from the snapshots.
Use it directly:
...
component) and validated in CI (`test.sh` Test 12) so they never drift from the
...
reference algorithm; the per-recipe half lives in
`tests/yocto/meta-rdk-halif/classes/rdk-halif-module.bbclass`.
...
`meta-rdk-halif` ships **one recipe per component** (`rdk-halif-<comp>`). The
version is not baked in: `HALIF_VERSION` defaults to the latest released snapshot,
and a build configuration selects a different version per component with
`HALIF_VERSION:pn-rdk-halif-<comp>`. Because a component's sibling dependencies
differ by version, each recipe carries a version→siblings map, so `DEPENDS`
follows whichever version is selected. With nothing overridden, every component
builds its latest snapshot — the latest cohort, which the generator asserts is
...
into the include the parent layer requires — the role, the destinations, and one
`HALIF_VERSION:pn-rdk-halif-<comp>` per component — validating that the cohort is
a consistent closure (a component's linked dependencies are pinned at the exact
linked version). Two worked examples ship under `tests/yocto/`:
