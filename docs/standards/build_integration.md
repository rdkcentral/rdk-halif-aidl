...
## Yocto: the `meta-rdk-halif` layer

The repository **is** a consumable Yocto layer at `meta-rdk-halif/`. Add it to
`BBLAYERS` and every released HAL snapshot is available as
`rdk-halif-<component>`, with inter-component `DEPENDS` derived from each
snapshot's `interface.yaml` `imports:`:
...
```bitbake
# In your image or a dependent recipe:
IMAGE_INSTALL:append = " rdk-halif-hdmicec"    # pulls rdk-halif-common via DEPENDS
```

BitBake orders the builds and stages each component's lib + headers into the
sysroot, so a dependent resolves `HALIF_LIB_DIR` / `HALIF_INCLUDE_DIR`
automatically — the failure a hand-written single recipe hits (a dependency not
yet staged when a dependent links) does not arise.

The recipes under `meta-rdk-halif/recipes-halif/` are **generated** from
`interface.yaml` by `scripts/gen_recipes.py` (one per component, latest released
version) and share the build logic in
`meta-rdk-halif/classes/rdk-halif-module.bbclass`. Do not hand-edit a recipe;
the dependency graph lives in `interface.yaml`. Pin `SRCREV` in the class (or via
`SRCREV:pn-rdk-halif-*`) to the released tag's commit for reproducible builds.
