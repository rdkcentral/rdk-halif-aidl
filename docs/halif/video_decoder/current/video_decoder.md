...
**Output side:** the Video Decoder shall deliver the final `IVideoDecoderControllerListener.onFrameOutput()` callback with `FrameMetadata.endOfStream = true` exactly once per decode session, strictly after any prior decoded-frame callbacks. When `endOfStream = true`, only that field is authoritative; all other fields of `FrameMetadata` (and the enclosing `frameAVBufferHandle` parameter) are undefined and MUST be ignored by the client.
...
When coded video frames are passed in through [AV Buffer](../../av_buffer/current/av_buffer.md) handles to `IVideoDecoderController.decodeBufferWithMetadata()` the `nsPresentationTime` parameter represents the video frame presentation time.
