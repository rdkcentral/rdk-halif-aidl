...
In every component that defines a listener, both processes contain a `Bp` proxy
*and* a `Bn` stub. That is 15 of the 21 components in `versions_released.yaml`;
`bootreason`, `common`, `deepsleep`, `deviceinfo`, `indicator` and `panel`
define none.
...
@nullable IAudioDecoder getAudioDecoder(
    in IAudioDecoder.Id decoderResourceId);                     // client calls

// IAudioDecoder
@nullable IAudioDecoderController open(
    in Codec codec, in boolean secure,
    in IAudioDecoderControllerListener audioDecoderControllerListener);  // client SUPPLIES an implementation
boolean registerEventListener(
    in IAudioDecoderEventListener audioDecoderEventListener);   // client SUPPLIES an implementation
```
...
**The unit a consumer selects is (component, version), never (component, role).**
...
A build pinned to `0.2.0` does not require a `0.2.0` server.
...
Below the major each side chooses freely. After a component adopts frozen-AIDL
discipline the interface is additive-only, and the vendor can hold a major while
the middleware versions independently.

Compatibility is checked at runtime rather than assumed.
[`halcompat::isCompatible()`](../../../common/current/halcompat.h)
...
A manifest pins the components a layer names; every dependency is then built at
the exact version its dependent links, because the build closure is keyed by
`(component, version)`. Where two consumers need different versions of the same
component, both are built side by side — `common@0.1.0.0` and `common@0.2.0.0`
coexist in one build, each serving the dependent that asked for it. Once
installed, the version in the library name and in the staged header path keeps
them apart; the layout is in
