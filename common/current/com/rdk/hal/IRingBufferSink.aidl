...
interface IRingBufferSink {
     ...
     *
     * If the underlying IRingBuffer is set up to use throttling behavior, this method will block until the requested
     * number of bytes are available for writing. If the underlying IRingBuffer is set up to use non-throttling
     * behavior, this method will return immediately with the number of bytes requested. It will thus override data
     * which has not yet been read by the consumer if the producer writes more data than the size of the ring buffer
     * without the consumer reading it.
     *
     ...
     *
     * @param size The number of bytes to acquire for writing.
     * @return An RingBufferAcquireResult containing the offset in the ring buffer where the producer can start writing
     *         data and the number of bytes that were actually acquired for writing. Null, if there's already space
     *         acquired by the producer that has not yet been released.
     */
     ...
     *
     * If size is less than the number of bytes acquired in the corresponding acquire call, the remaining bytes will be
     ...
     * @param size The number of bytes to release after writing.
     */
    void release(in RingBufferAcquireResult.Id id, in long size);

    /**
     * Abort the current call to acquire.
     *
     * As acquire might be blocking, this can be used to unblock a call to acquire that is currently blocked waiting for
     * blocks to become available for writing, for example when the producer is being stopped and should stop waiting
     * for blocks to become available for writing.
     *
     * By the nature of the operation, this method will not have any effect if there is no call to acquire currently
     * blocked. If the ring buffer is using non-throttling behavior, this method will not have any effect, as acquire
     * will never block in the first place.
     */
    void abortAcquire();
}
