...
public:
  ...
  float displacement = 0.000000f;
  inline bool operator!=(const EngineSpecs& rhs) const {
    return std::tie(engineType, horsepower, fuelType, displacement) != std::tie(rhs.engineType, rhs.horsepower, rhs.fuelType, rhs.displacement);
  }
  inline bool operator<(const EngineSpecs& rhs) const {
    return std::tie(engineType, horsepower, fuelType, displacement) < std::tie(rhs.engineType, rhs.horsepower, rhs.fuelType, rhs.displacement);
  }
  ...
  inline std::string toString() const {
    ...
    os << ", displacement: " << ::android::internal::ToString(displacement);
    os << "}";
    return os.str();
  }
};  // class EngineSpecs
