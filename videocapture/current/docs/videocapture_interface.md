...
| **HAL.VIDEOCAPTURE.17** | Shall keep a pool Dma-Buf's content and mapping valid for as long as any reference to it is held, reclaiming its memory only when the last one goes and never because a session ended; and shall write to no pool buffer after `stop()`, `close()`, loss of the source or client death.| A descriptor the client duplicated at `onPoolReady()`, and an image imported from one, are references in their own right. An implementation that frees or recycles on teardown instead of on the last reference breaks a client still drawing the frame it holds. The client needs no call on this interface to free a buffer and is offered none: it destroys its images and closes its descriptors. Negative test: hold a frame, end the session each way, sample what is held. |
