...
!!! info References
    |||
    |-|-|
    |**Interface Definition**|[videodecoder/current](https://github.com/rdkcentral/rdk-halif-aidl/tree/main/videodecoder/current)|
...
When coded video frames are passed in through [AV Buffer](../avbuffer/av_buffer.md) handles to `IVideoDecoderController.decodeBufferWithMetadata()` the `InputBufferMetadata.nsPresentationTime` field represents the video frame presentation time.
