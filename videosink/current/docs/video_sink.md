...
| **HAL.VIDEOSINK.10** | Shall declare in `Capabilities.supportsCapture` whether a capture session may bind to this sink, and where it declares `true` shall serve `com.rdk.hal.videocapture` the frame it would be presenting at that moment, under whatever presentation mode it is running, without altering its own display path. | The capture takes a view of the frames this sink has already scheduled; the scheduler and the timing policy stay here. A sink declaring `false` refuses `IVideoCapture.open()`. |
