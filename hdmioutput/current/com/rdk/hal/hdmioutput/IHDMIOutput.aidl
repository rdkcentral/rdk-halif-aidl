...
interface IHDMIOutput
{
     ...
     *
     * The input array must not be empty. Each entry must have its `property` key set.
     * On return, the corresponding `propertyValue` field is populated.
     *
     * @param[inout] propertyKVList  Holds keys on input and values on output.
     * @returns boolean
     * @retval true     All property values retrieved successfully.
     * @retval false    One or more keys were invalid, or input was empty.
     *
     * @see getProperty()
     */
