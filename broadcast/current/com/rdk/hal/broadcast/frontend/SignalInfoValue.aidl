 ...
 * SignalInfo values.
 *
 * These are the values that can be returned on status requests. It will always
 * be in sync with @ref SignalInfoProperty.
 *
 * TODO Many of these types are out of sync with the C++ HAL. We should also
 * consider wrapping them in parcelables so that they cannot be confused and
 * units are documented. Further, the list itself is probably out of sync with
 * the C++ HAL, as well.
 *
 * @author Jan Pedersen
...
union SignalInfoValue {
    ...
    /**
     * Pre Viterbi BER - The number of error bit per 1 billion bits before
     * correction
     */
    int preBer;
