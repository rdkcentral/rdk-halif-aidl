...
interface IDemuxController {
     ...
     *
     * The returned filters are not active by default. The filter has to be
     * configured with one or more PIDs to enable it.
     *
     * @param[in] filterType The type of filter to create.
     *
     * @returns IFilter or null on error (e.g. no filter available for the given
     *     type).
     */
     ...
     *
     * @exception binder::Status EX_ILLEGAL_STATE if the filter has not been
     * stopped.
     *
     * @pre The filter must have been stopped by calling IFilter.stop() before
     * calling this method.
     *
     * @see IFilter.stop()
     */
     ...
     *
     * This is used for writing data to the demux, e.g. for playing arecording or for feeding data from a network
     ...
     * @return The IRingBufferSink related to the demux's internal IRingBuffer.
     */
