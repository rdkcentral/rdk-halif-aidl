...
interface IDemux {
     ...
     *
     * Each demux might only be connected to one DemuxDataProvider. The connected
     * demux represented by the DemuxController can be used to set up multiple
     * filters, depending on the Capabilities.
     *
     ...
     *
     * @param[in] controller non-null DemuxController obtained from connect() on
     * the same Demux
     *
     ...
     *
     * The client takes ownership of the returned object. Resources are only bound
     * to the returned object; they are released when the injector is no longer
     * referenced.
     *
     * Only available when DemuxCapabilities.acceptsDataFromSoftware is true. The
     * number of simultaneous injector objects may be limited by the
     * implementation.
     *
     * @returns IDataFromSoftwareInjector or null on error (e.g. software input
     *     not supported,
